...
overlap_samples = int(samples_per_channel * 0.1)  # 10% overlap
...
overlap_buffer = np.zeros((overlap_samples, int(num_channels)), dtype=np.int16)
...
with wave.open(input_wav_file, 'rb') as wav:
    ...
    with open(fifo_path, 'wb') as fifo:
        ...
        while True:
            ...
            # Read next chunk, adjusted for overlap
            frames = wav.readframes(int(samples_per_channel - overlap_samples))

            # Convert frames to a numpy array and reshape to stereo format (2 channels)
            frames = np.frombuffer(frames, dtype=np.int16).reshape(-1, 2)

            # Concatenate overlap_buffer with the current frames
            combined_frames = np.concatenate((overlap_buffer, frames), axis=0)

            # Update overlap_buffer with the last 10% of the combined frames for next iteration
            overlap_buffer = combined_frames[-overlap_samples:]

            
            # Encode one block of data (now with overlap included)
            encoded_data = encode(combined_frames, target_entropy)

            # Calculate entropy and update gauge display (optional)
            in_entropy = calculate_16bit_entropy(combined_frames)
